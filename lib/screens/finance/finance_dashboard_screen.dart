...
import 'finance_payouts_screen.dart';
import 'finance_commissions_screen.dart';
import 'finance_refunds_screen.dart';
import 'finance_reports_screen.dart';
import 'finance_profile_screen.dart';
import 'finance_cash_collections_screen.dart';
...
class _FinanceDashboardScreenState extends State<FinanceDashboardScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final profileRes = await _api.get(
        context,
        '${ApiConfig.baseUrl}/api/finance/profile',
      );
      final statsRes = await _api.get(
        context,
        '${ApiConfig.baseUrl}/api/finance/stats',
      );
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ProfileHeader(
              username: _user?['username'] ?? 'Finance',
              role: 'Finance Team',
              email: _user?['email'] ?? '',
              phone: _user?['phone'] ?? '',
            ),
            ...
            GridView.count(
              ...
              children: [
                ...
                _buildStatCard('Admin Profit',
                    'TZS ${_stats?['totalAdminProfit'] ?? 0}',
                    Icons.account_balance, Colors.blue),
                _buildStatCard('Released', 'TZS ${_stats?['totalReleased'] ?? 0}',
                    Icons.check_circle, Colors.purple),
                _buildStatCard('Pending Escrow',
                    'TZS ${_stats?['totalPendingEscrow'] ?? 0}',
                    Icons.hourglass_empty, Colors.orange),
                _buildStatCard('Refund Requests',
                    '${_stats?['refundRequests'] ?? 0}', Icons.currency_exchange,
                    Colors.red),
                _buildStatCard('Total Purchases',
                    '${_stats?['totalPurchases'] ?? 0}', Icons.shopping_cart,
                    Colors.teal),
              ],
            ),
          ],
  ...
  void _showMoreMenu() {
    ...
    final moreItems = [
      {'label': 'Cash Collections', 'icon': Icons.attach_money, 'screen': const FinanceCashCollectionsScreen()},
      {'label': 'Refunds', 'icon': Icons.currency_exchange, 'screen': const FinanceRefundsScreen()},
      {'label': 'Reports', 'icon': Icons.assessment, 'screen': const FinanceReportsScreen()},
      {'label': 'Profile', 'icon': Icons.person, 'screen': const FinanceProfileScreen()},
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              _buildNavItem(Icons.attach_money, 'Payouts', 2),
              _buildNavItem(Icons.pie_chart, 'Commissions', 3),
              _buildMoreButton(),
            ],
          ),
        ),
  ...
  Widget _buildBody() {
    switch (_selectedIndex) {
      ...
      case 1:
        return const FinanceTransactionsScreen(showAppBar: false);
      case 2:
        return const FinancePayoutsScreen(showAppBar: false);
      case 3:
        return const FinanceCommissionsScreen(showAppBar: false);
      default:
        return _buildHomeContent();
    }
  }
        ...
        body: Column(
          children: [
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.account_balance, color: Color(0xFF0A2E5C)),
